"""Download ECOSTRESS L3T JET evapotranspiration granules.

This script searches NASA Earthdata for ECO_L3T_JET V002 granules within the
fixed Phoenix pilot domain and time window from config.py. Successful downloads
are saved under data/raw/ecostress_et/ and recorded in manifest.csv.
"""
...
DATASET = "ECOSTRESS Tiled Evapotranspiration Instantaneous and Daytime"
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Search and download ECO_L3T_JET V002 granules for Phoenix 2023."
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main() -> None:
    ...
    if args.sync_manifest_only:
        ...
        print(f"Found {len(local_paths)} local ECOSTRESS ET files in {output_dir}.")
    ...
    print("Searching for ECOSTRESS ET granules...")
